  ...
  return (
    <div className="flex flex-col items-center min-h-[calc(100vh-4rem)]">
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-4 w-full max-w-5xl mx-auto py-20 mt-10">
        <div className="inline-flex items-center rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-sm font-bold text-primary mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <span className="flex h-2 w-2 rounded-full bg-primary mr-2 animate-pulse"></span>
          KOC Arena v1.0 is Live
        </div>
        
        <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-foreground mb-6 animate-in fade-in zoom-in duration-700 delay-100">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300">
          {!isLoading && isAuthenticated ? (
            <Link href="/tournaments">
              <Button size="lg" className="h-14 px-8 text-lg bg-primary hover:bg-primary/90 text-primary-foreground shadow-xl shadow-primary/25 transition-all hover:scale-105 rounded-full">
          ...
          {!isAuthenticated && (
            <Link href="/login">
              <Button size="lg" variant="outline" className="h-14 px-8 text-lg border-primary/20 bg-card/50 backdrop-blur-md hover:bg-card transition-all hover:scale-105 rounded-full text-foreground">
                Sign In
              </Button>
            </Link>
          )}
      ...
      <div className="w-full max-w-6xl mx-auto grid md:grid-cols-3 gap-8 px-4 pb-24 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-500">
        <div className="glass p-8 rounded-3xl flex flex-col items-center text-center hover-lift">
          ...
          <h3 className="text-xl font-bold mb-3 text-foreground">Live Tournaments</h3>
        ...
        <div className="glass p-8 rounded-3xl flex flex-col items-center text-center hover-lift">
          <div className="h-16 w-16 rounded-2xl bg-accent/20 flex items-center justify-center mb-6 text-accent">
            <Swords className="h-8 w-8" />
          </div>
          <h3 className="text-xl font-bold mb-3 text-foreground">Instant Matchmaking</h3>
        ...
        <div className="glass p-8 rounded-3xl flex flex-col items-center text-center hover-lift">
          <div className="h-16 w-16 rounded-2xl bg-green-500/20 flex items-center justify-center mb-6 text-green-600">
            <Crown className="h-8 w-8" />
          </div>
          <h3 className="text-xl font-bold mb-3 text-foreground">Global Leaderboards</h3>
      ...
      {/* Footer using the large logo */}
      <footer className="w-full border-t border-border mt-auto py-8 bg-card/30">
        <div className="container mx-auto px-4 flex flex-col items-center">
          <img src="/complete-logo.webp" alt="Kingdom of Chess" className="h-16 w-auto object-contain mb-4 opacity-80 hover:opacity-100 transition-opacity" />
          <p className="text-sm text-muted-foreground text-center max-w-md">
            The live tournament and matchmaking platform.
          </p>
        </div>
      </footer>
    </div>
